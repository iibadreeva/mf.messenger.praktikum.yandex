...
export default class Chat extends Block<IContext> {
  ...
  render() {
    const {currentChat} = this.props as unknown as IContext;
    ...
    const templ = `
        ...
        <div class="messenger__chat__body">
          ...
          return `
            ...
            ${info.map(item => {
              return `
                <div class="${item.isMy ? 'messenger__chat__out' :'messenger__chat__in'}" >
                  ${item.text ? `${item.text}` : ''}
                  ${item.image ? `<div class="messenger__chat__image"><img src="${item.image}"></div>` : ''}
                  <span class="messenger__chat__time ${item.watch ? 'messenger__chat__time_active' :''}">
                    ${item.time}
                    ${item.isMy ?
